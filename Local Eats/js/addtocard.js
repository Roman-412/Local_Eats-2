...
function setTip(element) {
     document.querySelectorAll('.tip-btn').forEach(btn => btn.classList.remove('selected'));//அவங்கலிருந்து selected class remove பண்ணும்.
...
async function syncCartWithBackend() {
    ...
    if (!userId) return; //Login இல்லனா cart sync ஆகாது.

    let cart = JSON.parse(localStorage.getItem('cart')) || [];  //JSON string-ஐ object-ஆ மாற்ற JSON.parse() use பண்ணுறது.
    if (cart.length === 0) return; //Cart empty ஆ இருந்தா → function stop ஆகும்.

    try {
        let response = await fetch(`https://backend-2h2s.onrender.com/carts/user/${userId}`); //User-க்கு cart இருக்கா என்று check பண்ணும் API call.
        let backendCart;
        ...
        for (const item of cart) {
            const existingItem = backendCart.items.find(bi => (bi.food_id && bi.food_id === item.food_id) || bi.food_title === item.title);
            
            if (existingItem) {
                // Increment backend quantity by localStorage quantity
                const newQty = (existingItem.quantity || 0) + (item.quantity || 1);
                await fetch(`https://backend-2h2s.onrender.com/carts/${backendCart.id}/items/${existingItem.id}?quantity=${newQty}`, {
                    method: 'PUT'
                });
            } else {
